...
const predators = [
  ...
  [
    "img/animals/predators/bear-brown.jpg",
    1,
    "Бурый медведь",
    "sound/predators/bear.mp3",
  ],
  [
    "img/animals/predators/black-panther.jpg",
    2,
    "Черная пантера",
    "sound/predators/leopard.mp3",
  ],
  ...
  [
    "img/animals/predators/polar-bear.jpg",
    7,
    "Белый медведь",
    "sound/predators/bear.mp3",
  ],
...
class Slider {
  ...
  makeSlider() {
    ...
    if (this.animals[0].length === 5) this.sound(0);
    ...
    const slider = $("<div>")
      ...
      .append(
        $("<div>").html(
          '<svg viewBox="0 0 436 467" class="game__start-test"><use href="#start-test" /></svg>'
        )
      )
    ...
    for (let animal of this.animals) {
      ...
      element.append(
        $("<img>", { src: `${animal[0]}`, class: "slider__img", alt: `${animal[2]}` })
      );
    ...
    $(".slider__btn").on("click", () => {
      if (slider_pos !== this.animals.length - 1) this.sound(slider_pos);
  ...
  play() {
    ...
    const result = $("<div>")
      .addClass("test__result result")
      .css("background-image", `url(${this.animals.slice(-1)[0][0]})`);
    for (let i = 0; i < 3; i++) {
      //создать 3 звезды
      ...
      $(svg_star).addClass("result__star").append($(use_tag).attr("href", "#star"));
    ...
    let count_result = (answers) => {
      //расчет награды за тест
    ...
    function randomNumber(arr, i, max) {
      //генератор зверей для вопроса.
      if (arr[i] == null) {
        let random = ~~(Math.random() * max);
        if (arr.includes(random)) randomNumber(arr, i, max);
        else arr[i] = random;
      }
      return arr;
    }
    ...
    function createBalloon(parent) {
      ...
      for (let i = 0; i < 15; i++) {
        ...
        balloon
          .addClass("balloon__item")
          .css({
            left: `${~~(Math.random() * 87)}%`, //Случайное гориз. полож.
            //  fill: `rgb(${~~(Math.random() * 255)},
            //  						${~~(Math.random() * 255)},
            //  						${~~(Math.random() * 255)}` //Случ. цвет
            filter: `hue-rotate(${~~(Math.random() * 360)}deg)`,
          })
          .append(use.addClass("balloon__item-path").attr("href", "#balloon"));
        setTimeout(
          () => $(parent).append(balloon), //Разброс по времени появления шариков
          ~~(Math.random() * 7000)
        );
      }
    }
    ...
    function getElementsFromPoint(x, y) {
      let element = 0,
        elements = []; // массив элементов под курсором
      let start = performance.now();
      while (performance.now() - start < 10) {
        element = document.elementFromPoint(x, y); //Получить эл. под курсором.
        if (!element || element.classList.contains("result")) break; //если null или дошли до контейнера - остановить цикл
        element.style.visibility = "hidden"; //скрыть объект
        elements.push(element); //добавить эл. в массив
      }
      for (const elem of elements) elem.style.visibility = ""; //вернуть видимость объектов
      return elements;
    }
    ...
    let build_task = (index) => {
      if (index === this.animals.length - 1) {
        //this.animals.length - 1 все вопросы отвечены
        ...
        if (!isGameFinished) {
          //Условие для однократного исполнения блок инструкций
          isGameFinished = 1;
          setTimeout(() => {
            // Скрыть звезды
    ...
    test_body.on("click", function (evt) {
      ...
      if (animals_arr[target_index] === index) {
        ...
        let sound_length = 3;
        if (animals_list[index].length === 5) sound_length = this_obj.sound(index);
        //добавить анимацию верного ответа
        target
          .css({
            // "transform-origin": trans_origin_map[target_index],
            "animation-duration": sound_length + "s",
          })
          .addClass("test__img_active");
        //Проверка, что верный ответ получен с первой попытки.
        if ($(".wrong").length === 0) answers[1]++;
        answers[0]++;
    ...
    result.on("click", function (evt) {
      ...
      for (const tag of balloons) {
        ...
        if ($tag.prop("tagName") === "use" && !parent.hasClass("result__star")) {
          //анимация взрыва
          parent
            .html('<image href="img/boom.svg" class="balloon__boom"/>')
            .css("filter", "none");
          //убрать анимацию
      ...
      setTimeout(() => {
        ...
        if (result.children().length < 4) {
          ...
          if (!this_game_node.hasClass("active")) this_game_node.css("animation", "none");
          setTimeout(() => {
...
$(".animals__sub-btn").on("click", function (evt) {
  ...
  $("html, body")
    .stop()
    .animate({ scrollTop: $(link).offset().top - 200 + "px" }, 300);
});
